  ...
  \begin{algorithmic}[1]
    ...
    \WHILE{$\nu \leq \nu_{\max}$}
      \FOR{$k \leftarrow 0,\dots, 2^\nu-1$}
        \STATE $\mathcal{S}_V^k \leftarrow \texttt{scan\_map}(\bm{M}, \hat{\bm{p}}_1 + [0,0,k \gamma 2^{-\nu}])$
        \STATE $\xi_1^k \leftarrow \arg\max \mathcal{F}^{-1}\{Q_{\mathcal{S}_V^k, \mathcal{S}_1}\}$ \hspace{1.55cm} (eq. \ref{eq:Q})
        \STATE $\hat{\bm{p}}_1^k \leftarrow \hat{\bm{p}}_1^k + [0,0, \xi_1^k \gamma + k \gamma 2^{-\nu}]$ \hspace{0.75cm}(sec. \ref{subsec:method_orientation_correction})
        \STATE $\hat{\bm{p}}_1^{k} \leftarrow \texttt{translate}(\hat{\bm{p}}_1^k, \bm{M}, \mathcal{S}_1, 1)$
      ...
      \STATE $\hat{\bm{p}}_1^\prime \leftarrow \texttt{translate}(\hat{\bm{p}}_1^\prime, \bm{M}, \mathcal{S}_1, I);$  \hspace{0.65cm}(sec. \ref{subsec:method_location_correction})
